...
class EditarPostPerdidoPage extends StatefulWidget {
  const EditarPostPerdidoPage({super.key, required this.pid});
  final String pid;
  @override
  State<EditarPostPerdidoPage> createState() => _EditarPostPerdidoPageState();
}

class _EditarPostPerdidoPageState extends State<EditarPostPerdidoPage> {
  List<Map> datos = [];
...
class _T1bodyState extends State<T1body> {
  ...
  Future<void> finalizarPost(String postid, BuildContext ctx) async {
    ...
    var respuesta = await http.post(Uri.parse(PERDIDOS_FIN), body: mipost);
      ...
      child: Column(
        children: [
          ...
          const Padding(
            ...
            child: Text(
              "Si ya encontro a su mascota por favor haga clic en el boton para actualizar el estado de la publicacion a LOCALIZADO",
...
Future<Map> getReq(String a, BuildContext context) async {
  ...
  var result = await http.post(Uri.parse(PERDIDOS_SHOW), body: JsonBody);
...
class Tab_5 extends StatelessWidget {
  ...
  Future<void> crearPost(Map info, String postid, BuildContext ctx) async {
    ...
    var respuesta = await http.post(Uri.parse(PERDIDOS_EDIT), body: mipost);
        ...
        const Padding(
          ...
          child: Column(
            children: [
              ...
              Text(
                "¿Desea actualizar publicacion ahora?",
                style: styleNormal,
              )
            ],
          ),
          ...
          ElevatedButton.icon(
              onPressed: () {
                ...
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (context) => PostPerdidoPage(
                              postid: model.postid,
                            )));
              },
      ...
      for (var i = 0; i < imagenes[j]["imgs"].length; i++) {
        ims.add(
          SizedBox(
            ...
            height: 70,
            child: Image.network(
              "$PERDIDOS_IMG/${imagenes[j]["imgs"][i]}",
              fit: BoxFit.cover,
            ),
          ),
        );
