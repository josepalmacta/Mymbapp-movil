...
class EditarPostAdopcionPage extends StatefulWidget {
  const EditarPostAdopcionPage({super.key, required this.pid});
  final String pid;
  @override
  State<EditarPostAdopcionPage> createState() => _EditarPostAdopcionPageState();
}

class _EditarPostAdopcionPageState extends State<EditarPostAdopcionPage> {
  List<Map> datos = [];
...
class _T1bodyState extends State<T1body> {
  ...
  Future<void> finalizarPost(String postid, BuildContext ctx) async {
    ...
    var respuesta = await http.post(Uri.parse(ADOPCION_FIN), body: mipost);
      ...
      child: Column(
        children: [
          ...
          const Padding(
            ...
            child: Text(
              "Si logro concretar la adopcion por favor haga clic en el boton para actualizar el estado de la publicacion a ADOPTADO",
...
class Tab_5 extends StatelessWidget {
  ...
  Future<void> crearPost(Map info, String postid, BuildContext ctx) async {
    ...
    var respuesta = await http.post(Uri.parse(ADOPCION_EDIT), body: mipost);
          ...
          ElevatedButton.icon(
              onPressed: () {
                ...
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (context) => PostAdopcionPage(
                              postid: model.postid,
                            )));
              },
              ...
              Column(
                ...
                children: [
                  ...
                  const Padding(
                    ...
                    child: Text("Ingrese los requisitos para la adopcion:"),
                  ),
                  SizedBox(
                    height: 150,
                    child: TextFormField(
      ...
      for (var i = 0; i < imagenes[j]["imgs"].length; i++) {
        ims.add(
          SizedBox(
            ...
            height: 70,
            child: Image.network(
              "$ADOPCION_IMG/${imagenes[j]["imgs"][i]}",
              fit: BoxFit.cover,
            ),
          ),
        );
