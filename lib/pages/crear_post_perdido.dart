...
class CrearPostPerdidoPage extends StatefulWidget {
  const CrearPostPerdidoPage({super.key});

  @override
  State<CrearPostPerdidoPage> createState() => _CrearPostPerdidoPageState();
}

class _CrearPostPerdidoPageState extends State<CrearPostPerdidoPage> {
  List<Map> datos = [];
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          const Text(
            "Agregue informacion sobre su mascota.",
            style: styleBold,
          ),
          const SizedBox(
            height: 20,
    ...
    return Consumer<MyModel>(builder: (context, value, child) {
      return Padding(
        ...
        child: Column(
          children: [
            const Text(
              "¿En que barrio perdio a su mascota?",
              style: styleBold,
            ),
            const SizedBox(
              height: 20,
...
class Tab_5 extends StatelessWidget {
  ...
  Future<void> crearPost(Map info, List masco, BuildContext ctx) async {
    ...
    mipost["recomp"] = mipost.containsKey("recompensa");

    var body = json.encode(mipost);

    log("aaaaaaaaaaaaaaaaaaaa");

    var respuesta = await http.post(Uri.parse(PERDIDOS_ADD),
...
class _Tab_4State extends State<Tab_4> {
  ...
  void initState() {
    ...
    final model = Provider.of<MyModel>(context, listen: false);
    setState(() {
      ena = model.post.containsKey("recompensa");
    });
  }
  ...
  crearPost() {
    ...
    if (ena && recompensa_controller.text.isNotEmpty) {
      campos.addAll({"recompensa": recompensa_controller.text});
    } else {
      if (model.post.containsKey("recompensa")) {
        model.eliminarCampos({"recompensa": "h"});
      }
    }
                    ...
                    children: [
                      Checkbox(
                          ...
                          onChanged: ((valor) {
                            ...
                            if (!valor!) {
                              recompensa_controller.text = "";
                            }
                          })),
                      const Text("Ofrezco recompensa"),
                    ],
                  ),
                  TextFormField(
                    enabled: ena,
                    controller: recompensa_controller,
                    decoration: inputStyle("Recompensa", Icons.card_giftcard),
                  ),
                  const SizedBox(
                    height: 20,
                  ),
            ...
            Positioned(
              ...
              child: Badge(
                ...
                label: const Padding(
                  ...
                  child: Text("Visto por ultima vez"),
                ),
              ),
            ),
            Positioned(
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                decoration: inputStyle("Nombre de la mascota", Icons.pets),
              ),
              const SizedBox(
                height: 20,
              ),
              ...
              TextFormField(
                ...
                validator: validar,
                decoration: inputStyle("Descripcion de la mascota", Icons.info),
              ),
            ],
          )),
    );
